...
function Tabel() {
            ...
            try {
                if (selectedImageIdz) {
                    const response = await axios.get(
                        `http://localhost:8000/api/komputer/${selectedImageIdz}`,
                    );
    ...
    const handleBoxClick = (day, hour) => {
        if (!buttonAPressed) {
            ...
            if (peminjaman[`${day}-${hour}`] === 0) {
                ...
                const previousColor =
                    peminjaman[`${day}-${hour}`] === 1 ? 1 : 0; // Nilai merah (1) atau hijau (0) sebelumnya
                setPeminjaman((prevPeminjaman) => ({
                    ...prevPeminjaman,
    ...
    const handleButtonAClick = async () => {
        ...
        if (alasanPeminjaman.trim() !== "") {
            ...
            const updatedAlasanPeminjamanArray = [
                ...alasanPeminjamanArray,
                alasanPeminjaman,
            ];
            ...
            console.log(
                "Array Alasan Peminjaman:",
                updatedAlasanPeminjamanArray,
            );
        } else {
        ...
        for (let i = 0; i < tabelKuning.length; i++) {
            ...
            try {
                const response = await fetch(
                    "http://localhost:8000/api/addpeminjaman",
                    {
                        method: "POST",
                        body: formData,
                    },
...
export default function Homepage(proops) {
    ...
    return (
        ...
        >
            ...
            <div className="overflow-x-autom mt-3">
                <tabel>
                    <Tabel />
                </tabel>
            </div>
